function [sensitivity,range,correlation,cmd,regSeries,tsi,Yr] = ...
    calccmdsensitivity(yrSt,yrEnd,yrCensor,cmdMethod,diffwshift,tsiChoice,TChoice,lonCov,ciFlag, ci, iter, plotFlag)
...
%TChoice = ;            %1 HadCRUT4 (1850 to present) 2 NCEP (1948-present)                        
                        %3 GISS 4 HadCRUT3. Note 1 and 3 have not been fully validated
                        %through comparison to relevant LDA or CMD papers
                        %due to uncertainty in how to exclude missing grid
                        %boxes
if ~exist('TChoice')
    TChoice = 2;
end                        
...
if TChoice ~= 2
    ...
    if isempty(var) || size(var,3) < 2
        sensitivity = NaN;correlation=NaN;cmd=NaN;regSeries=NaN;tsi=NaN;
        return
    end
else
...
function [var,Yr,yI] = returnvalidlat(var,Yr,Y,lonCov)
    ...
    obsNum = squeeze(nansum(obsNum > 4,3)); %number of years missing fewer than X obs
